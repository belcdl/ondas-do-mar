...
async def test_create_checkout_session_returns_url(
    ...
    response = await client.post(f"/api/v1/bookings/{booking['id']}/checkout-session")
...
async def test_create_checkout_session_booking_not_found(
    ...
    _patch_stripe_checkout(monkeypatch)
    response = await client.post(f"/api/v1/bookings/{uuid.uuid4()}/checkout-session")
...
async def test_stripe_webhook_invalid_signature_returns_400(
    ...
    response = await client.post(
        "/api/v1/webhooks/stripe", content=b"{}", headers={"stripe-signature": "not-real"}
    )
...
async def test_stripe_webhook_completed_event_confirms_booking(
    ...
    _patch_stripe_checkout(monkeypatch)
    checkout_response = await client.post(f"/api/v1/bookings/{booking['id']}/checkout-session")
    ...
    webhook_response = await client.post(
        "/api/v1/webhooks/stripe", content=b"{}", headers={"stripe-signature": "sig"}
    )
    assert webhook_response.status_code == 200

    booking_response = await client.get(f"/api/v1/bookings/{booking['id']}", headers=admin_headers)
