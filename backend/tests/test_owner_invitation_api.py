...
async def test_invite_owner_requires_admin(
    ...
    response = await client.post(f"/owners/{owner['id']}/invite")
...
async def test_invite_owner_not_found_returns_404(
    ...
    response = await client.post(f"/owners/{uuid.uuid4()}/invite", headers=admin_headers)
...
async def test_invite_already_onboarded_owner_returns_409(
    ...
    owner = await _create_owner(client, admin_headers)
    invite_response = await client.post(f"/owners/{owner['id']}/invite", headers=admin_headers)
    ...
    response = await client.post(f"/owners/{owner['id']}/invite", headers=admin_headers)
...
async def test_full_invitation_flow_scopes_owner_correctly(
    ...
    invite_response = await client.post(f"/owners/{owner_a['id']}/invite", headers=admin_headers)
    ...
    own_profile = await client.get(f"/owners/{owner_a['id']}", headers=owner_headers)
    assert own_profile.status_code == 200
    assert own_profile.json()["id"] == owner_a["id"]

    other_profile = await client.get(f"/owners/{owner_b['id']}", headers=owner_headers)
    assert other_profile.status_code == 403

    me_response = await client.get("/auth/me", headers=owner_headers)
